...
export class TradeService {
  ...
  static async getTrades(): Promise<{ trades: Trade[]; error: string | null }> {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      ...
      if (cachedTrades) {
        console.log('📦 Using cached trades:', cachedTrades.length);
      ...
      return { trades: [], error: 'An unexpected error occurred' };
    }
  }
  ...
  static async updateTrade(tradeId: string, updates: Partial<Omit<Trade, 'id' | 'createdAt'>>): Promise<{ trade: Trade | null; error: string | null }> {
    try {
      ...
      if (updates.date !== undefined) {
        ...
        console.log('🗓️ Date update:', { originalDate: updates.date, convertedDate: dateValue });
      }
  ...
  private static validateAndCleanTrade(trade: Trade): Omit<Trade, 'id' | 'createdAt'> | null {
    ...
    if (!trade.type || !partyName) {
      console.log('Trade validation failed - missing required fields:', {
        id: trade.id,
        type: trade.type,
    ...
    if (finalQuantity === 0 && finalAmount === 0) {
      console.log('Trade validation failed - no meaningful data for Buy/Sell trade:', {
        id: trade.id,
        type: trade.type,
